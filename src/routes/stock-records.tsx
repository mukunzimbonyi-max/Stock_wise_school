...
function StockRecords() {
  ...
  const exportCsv = () => {
    const header = [
      "Date", "Food Item", "Started With", "Received", "Supplier", "Supplier Signature",
      "Provided", "Cook", "Cook Signature", "Destroyed", "Thrown Away", "Total Used",
      "Remaining", "Explanation",
    ];
    const body = filtered.map((r) => [
      r.date, r.foodItem, r.startedWith, r.received, r.supplierName, r.supplierSignature,
      r.provided, r.cookName, r.cookSignature, r.destroyed, r.thrownAway, totalUsed(r),
      remaining(r), `"${r.explanation.replace(/"/g, "'")}"`,
    ]);
    ...
    >
        ...
        <div className="card-surface p-5">
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
            <div className="space-y-1.5">
              ...
              <Input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="Food, supplier, cook..." />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={item} onValueChange={(v) => { setItem(v); setPage(1); }}>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <Button asChild>
              <Link to="/add-stock"><PlusCircle className="mr-2 h-4 w-4" /> Add New Stock Record</Link>
            </Button>
            ...
            <table className="w-full min-w-[1500px] text-sm">
              <thead className="bg-muted/60">
                <tr className="text-left text-xs uppercase tracking-wide text-muted-foreground">
                  {["Date","Food Item","Started With","Received","Supplier Name","Supplier Signature","Provided","Cook Name","Cook Signature","Destroyed","Thrown Away","Total Used","Remaining Stock","Explanation","Actions"].map((h) => (
              ...
              <tbody>
                {rows.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="border-t border-border/70 transition-colors hover:bg-muted/40">
                      ...
                      <td className="whitespace-nowrap px-4 py-3 italic text-muted-foreground">{r.supplierSignature}</td>
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${rem < LOW_STOCK_THRESHOLD ? "bg-warning/15 text-warning" : "bg-success/15 text-success"}`}>
                          {rem} {r.unit}
                        </span>
                      </td>
                      <td className="max-w-[240px] truncate px-4 py-3 text-muted-foreground">{r.explanation}</td>
                      <td className="whitespace-nowrap px-4 py-3">
                        <div className="flex gap-1">
                          <Button size="icon" variant="ghost" aria-label="View" onClick={() => setViewing(r)}><Eye className="h-4 w-4" /></Button>
                          <Button size="icon" variant="ghost" aria-label="Edit" onClick={() => setEditing({ ...r })}><Pencil className="h-4 w-4" /></Button>
                          <Button size="icon" variant="ghost" aria-label="Delete" className="text-destructive hover:text-destructive" onClick={() => setDeleting(r)}><Trash2 className="h-4 w-4" /></Button>
                        </div>
                      </td>
                    </tr>
                  );
                ...
                {rows.length === 0 && (
                  <tr><td colSpan={15} className="px-4 py-10 text-center text-muted-foreground">No records match your filters.</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border px-4 py-3">
            ...
            <div className="flex gap-1">
              ...
              {Array.from({ length: pages }, (_, i) => (
                <Button key={i} size="sm" variant={current === i + 1 ? "default" : "outline"} onClick={() => setPage(i + 1)}>{i + 1}</Button>
              ))}
      ...
      <Dialog open={!!viewing} onOpenChange={(o) => !o && setViewing(null)}>
        <DialogContent className="max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{viewing?.foodItem} — {viewing?.date}</DialogTitle>
          ...
          {editing && (
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="space-y-1.5">
                ...
                <Select value={editing.foodItem} onValueChange={(v) => setEditing({ ...editing, foodItem: v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
              ...
              {([
                ...
                <div key={key} className="space-y-1.5">
                  <Label>{label}</Label>
                  <Input type="number" min={0} value={editing[key]} onChange={(e) => setEditing({ ...editing, [key]: Number(e.target.value) || 0 })} />
                </div>
              ))}
              <div className="space-y-1.5">
                <Label>Supplier name</Label>
                <Input value={editing.supplierName} onChange={(e) => setEditing({ ...editing, supplierName: e.target.value })} />
              </div>
              <div className="space-y-1.5 sm:col-span-2">
                <Label>Explanation</Label>
                <Input value={editing.explanation} onChange={(e) => setEditing({ ...editing, explanation: e.target.value })} />
              </div>
